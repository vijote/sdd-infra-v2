...
- [x] T011 [Stage 3: Verify] AC-004: kubeadm init completed — control plane node Ready (verified via clean bootstrap log + join command publication; the SSM Run Command `kubectl wait` path was not separately executed) (Depends on T010)
- [x] T012 [Stage 3: Verify] AC-005: API server up AND join command published (`aws ssm get-parameter --name /sdd-k8s-platform/kubeadm-join-command --with-decryption --query 'Parameter.Value' --output text | grep -q 'kubeadm join'`) (Depends on T011)
