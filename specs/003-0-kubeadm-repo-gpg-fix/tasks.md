...
- [ ] T003 [Stage 2: Verify] AC-002: Repo file declares the GPG key (`grep -q 'gpgkey=https://pkgs.k8s.io/core:/stable:/v1.28/rpm/repodata/repomd.xml.key' terraform/modules/control-plane/bootstrap.sh`) (Depends on T002)
- [ ] T004 [Stage 2: Verify] AC-003: Repo file enables GPG checking (`grep -q 'gpgcheck=1' terraform/modules/control-plane/bootstrap.sh`) (Depends on T003)
- [ ] T005 [Stage 2: Verify] AC-004: The bare `--add-repo` line is removed (`! grep -q 'config-manager --add-repo' terraform/modules/control-plane/bootstrap.sh`) (Depends on T004)
