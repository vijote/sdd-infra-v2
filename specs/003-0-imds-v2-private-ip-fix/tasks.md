...
- [x] T003 [Stage 2: Verify] AC-002: IMDSv2 token is requested (`grep -q 'X-aws-ec2-metadata-token-ttl-seconds' terraform/modules/control-plane/bootstrap.sh`) (Depends on T002)
- [x] T004 [Stage 2: Verify] AC-003: The metadata call passes the token (`grep -q 'X-aws-ec2-metadata-token: ${IMDS_TOKEN}' terraform/modules/control-plane/bootstrap.sh`) (Depends on T003)
- [x] T005 [Stage 2: Verify] AC-004: KubeletConfiguration warning eliminated — SUPERSEDED: the KubeletConfiguration document was removed entirely by 003-0-kubeadm-preflight-sysctl-fix, so the ignored-document warning is gone (the original `kubelet.k8s.io/v1beta1` GVK line no longer exists) (Depends on T004)
