# Technical Quality Checklist: Flannel SSM Params JSON Fix

**Purpose**: Validate technical rigor, infrastructure contracts, and machine-verifiability before planning/execution.
**Created**: 2026-09-06
**Feature**: [Flannel SSM Params JSON Fix](../spec.md)

## 1. Technical Contract Completeness
- [x] CHK001 Is the `--parameters` contract (exact before/after escaping, exact JSON shape) explicitly declared?
...
- [x] CHK004 Is the root cause (`\\\"` over-escape → AWS CLI receives `\"` → invalid JSON → ParamValidation) documented and the fix mapped to it?
- [x] CHK005 Is the deployment method (existing terraform-apply workflow) explicitly stated?

## 2. Infrastructure & Security Hygiene
- [x] CHK006 Is the escaping trace (file → heredoc → bash → AWS CLI) justified step by step?
- [x] CHK007 Are the existing `interpreter`, SSM send-command flags, and poll loop left byte-for-byte unchanged?
...
- [x] CHK012 Are all contract requirements quantified (exact `\"` present, exact `\\\"` absent)?
