...
- [x] T003 [Stage 2: Verify] AC-002: `--parameters` uses single-backslash JSON quotes (`grep -qF 'commands=[\"' terraform/environments/dev/main.tf`) (Depends on T002)
- [x] T004 [Stage 2: Verify] AC-003: Triple-backslash over-escape removed (`! grep -qF '\\\"' terraform/environments/dev/main.tf`) (Depends on T003)
