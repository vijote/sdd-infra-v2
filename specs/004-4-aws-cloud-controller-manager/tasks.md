...
- [x] T003 [Stage 1: Bootstrap] Add `null_resource.apply_aws_ccm` to `terraform/environments/dev/main.tf`: `depends_on = [null_resource.apply_app_frontend_ingress]`, `triggers.ccm_version = "eks-v1.28.0"`, local-exec (bash interpreter) mirroring `apply_app_backend` — SSM command: (1) base64-apply `aws-ccm.yaml`, (2) `kubectl annotate svc ingress-nginx-controller -n ingress-nginx service.beta.kubernetes.io/aws-load-balancer-subnets='<%%PUBLIC_SUBNETS%%>' --overwrite` (replace with `join(",", module.vpc.public_subnet_ids)`), (3) `kubectl rollout status deployment/aws-cloud-controller-manager -n kube-system --timeout=300s`
