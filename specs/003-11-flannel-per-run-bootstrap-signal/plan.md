...
| `terraform/modules/worker-nodes/main.tf` | Modify | Change `user_data` from `file()` to `templatefile()` to inject `var.control_plane_instance_id` |
| `terraform/modules/worker-nodes/bootstrap.sh` | Modify | Add `%{control_plane_instance_id}` placeholder; gate the join on the instance-ID param matching before fetching + `eval`ing the join command |
...
3. **Worker bootstrap (agent)** — `templatefile` injection + instance-ID gate before `eval`.
...
- **AC-005**: `grep -qF 'templatefile' terraform/modules/worker-nodes/main.tf`
...
| `templatefile` breaks on a malformed placeholder | `terraform validate` (AC-001) catches it; the `%{...}` syntax is explicit |
