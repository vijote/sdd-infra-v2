...
- [x] Worker bootstrap change is byte-level specified (instance-ID gate before `eval`, `replace()` token injection)
...
- [x] The `replace()` token-injection syntax (`%%CONTROL_PLANE_INSTANCE_ID%%`) is specified, and why `templatefile` is unsuitable (bash `${...}` + `%{...}` control syntax)
...
- [x] Risk: the `%%CONTROL_PLANE_INSTANCE_ID%%` token collides with script content — mitigated by the token being unique (appears once) and `replace()` being a plain string operation (no template engine)
