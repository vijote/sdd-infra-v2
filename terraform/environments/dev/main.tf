...
resource "null_resource" "apply_route53_record" {
  ...
  triggers = {
    ...
    instance_id = module.control_plane.control_plane_instance_id # 004-10: re-apply on cluster recreation
    route53_ref = "route53-wildcard+typo-fix+alias-health" # 009-1: node_route53 wildcard; 009-1b: CLI typo fix; 009-1c: add required EvaluateTargetHealth to ALIAS (command string is not in null_resource state)
  }
