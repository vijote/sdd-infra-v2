...
resource "null_resource" "apply_route53_record" {
  ...
  triggers = {
    ...
    instance_id = module.control_plane.control_plane_instance_id # 004-10: re-apply on cluster recreation
    route53_ref = "route53-wildcard+typo-fix" # 009-1: re-run SSM command after node_route53 wildcard; 009-1b: re-run after create-route53-record.sh typo fix (command string is not in null_resource state)
  }
